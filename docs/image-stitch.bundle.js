 ...
 * Generated on 2025-10-28T22:50:50.837Z
...
class FileInputAdapter {
    ...
    async *scanlines() {
        ...
        try {
            ...
            // Stream scanlines
            const bytesPerPixel = getBytesPerPixel(header.bitDepth, header.colorType);
            const scanlineLength = Math.ceil((header.width * header.bitDepth * this.getSamplesPerPixel(header.colorType)) / 8);
            ...
            for (let line = 0; line < header.height; line++) {
                ...
                const filterType = decompressedData[offset];
                const filteredData = decompressedData.slice(offset + 1, offset + bytesPerLine);
                const unfilteredData = unfilterScanline(filterType, filteredData, previousScanline, bytesPerPixel);
...
class Uint8ArrayInputAdapter {
    ...
    async *scanlines() {
        ...
        for (let line = 0; line < header.height; line++) {
            ...
            const filterType = decompressedData[dataOffset];
...
//# sourceMappingURL=png-input-adapter.js.map
 ...
 */
export async function* concatPngsStreaming(options) {
 ...
 */
export function concatPngsToStream(options) {
    const concatenator = new StreamingConcatenator(options);
...
//# sourceMappingURL=png-concat.js.map